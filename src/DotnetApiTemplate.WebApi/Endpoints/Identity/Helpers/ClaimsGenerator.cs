...
namespace DotnetApiTemplate.WebApi.Endpoints.Identity.Helpers;
...
public static class ClaimsGenerator
{
    public static Dictionary<string, IEnumerable<string>> Generate(User user, IAES _iaes)
    {
        ...
        {
            ["xid"] = new[] { user.Id.ToString() },
            ["usr"] = new[] { user.Username },
            [ClaimTypes.Name] = new[] { user.Id.ToString() }
        };
        ...
        if (!string.IsNullOrWhiteSpace(user.FullName))
            claims.Add(ClaimTypes.GivenName, new[] { user.FullName });
