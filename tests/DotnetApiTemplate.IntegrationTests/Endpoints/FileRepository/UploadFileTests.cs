...
using DotnetApiTemplate.WebApi.Endpoints.FileRepository;
...
namespace DotnetApiTemplate.IntegrationTests.Endpoints.FileRepository;
...
public class UploadFileTests
{
    ...
    public async Task UploadFile_ShouldBe_Correct()
    {
        ...
        var data = await scope.ServiceProvider.GetRequiredService<IDbContext>().Set<Domain.Entities.FileRepository>()
            .Where(e => e.Id == result.Value.FileId).FirstOrDefaultAsync(CancellationToken.None);
