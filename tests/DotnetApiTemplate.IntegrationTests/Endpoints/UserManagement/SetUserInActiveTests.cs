...
using DotnetApiTemplate.Shared.Abstractions.Enums;
using DotnetApiTemplate.WebApi.Endpoints.UserManagement;
...
namespace DotnetApiTemplate.IntegrationTests.Endpoints.UserManagement;
...
public class SetUserInActiveTests
{
    ...
    public async Task SetUserInActive_Should_Do_Correctly()
    {
        ...
        //first time get
        var user = await dbContext.Set<User>().Where(e => e.Id == request.UserId)
        ...
        //second time get
        user = await dbContext.Set<User>().Where(e => e.Id == request.UserId)
            .FirstOrDefaultAsync(CancellationToken.None);
        user.ShouldNotBeNull();
